    ...
    async fn handle_cmd(&mut self, behaviour: &mut Self::Behaviour, cmd: Self::Command) {
        match cmd {
            IdentifyCommand::RequestIdentify { peer_id } => {
                debug!(
                    "🔄 [IdentifyHandler] Processing RequestIdentify command for peer: {:?}",
                    peer_id
                );
                ...
                info!(
                    "📡 [IdentifyHandler] Identify information will be exchanged automatically on connection with peer: {:?}",
                    peer_id
                );
            }
            IdentifyCommand::SendIdentify { peer_id } => {
                debug!(
                    "🔄 [IdentifyHandler] Processing SendIdentify command for peer: {:?}",
                    peer_id
                );
                // Identify protocol automatically sends identify information
                info!(
                    "📤 [IdentifyHandler] Identify information will be sent automatically to peer: {:?}",
                    peer_id
                );
            }
        }
