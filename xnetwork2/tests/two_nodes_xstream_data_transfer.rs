...
mod utils;
use utils::{wait_for_event, spawn_auto_respond_por_task};


/// Тестирует полную передачу данных через XStream между двумя нодами с проверкой целостности
/// Весь тест должен укладываться в 10 секунд (с учетом аутентификации)
#[tokio::test]
async fn test_two_nodes_xstream_complete_data_transfer_in_10_seconds() {
    println!("🧪 Запуск полноценного теста XStream с реальным обменом данных (10 секунд)...");
    ...
    let result = timeout(Duration::from_secs(10), async {
                        ...
                        match stream.read_to_end().await {
                            Ok(data) => {
                                ...
                                match stream.write_all(part1.as_bytes().to_vec()).await {
                                    Ok(_) => {
                                        println!("✅ Нода1 успешно отправила первую часть данных");
                                    }
                                    Err(e) => {
                                ...
                                match stream.write_all(part2.as_bytes().to_vec()).await {
                                    Ok(_) => {
                                        println!("✅ Нода1 успешно отправила вторую часть данных");
                                    }
                                    Err(e) => {
                            ...
                            Err(e) => {
                                panic!("❌ Нода1 не смогла прочитать данные из XStream: {:?}", e);
                            }
                        }
                        break;
                    }
                    _ => continue, // Игнорируем другие события
                }
            }
        });
        ...
        por_task_node1.await
            .expect("❌ Задача PoR для ноды1 завершилась с ошибкой (join)")
            .expect("❌ Задача PoR для ноды1 завершилась с ошибкой (task)");
        por_task_node2.await
            .expect("❌ Задача PoR для ноды2 завершилась с ошибкой (join)")
            .expect("❌ Задача PoR для ноды2 завершилась с ошибкой (task)");
        
        println!("✅ Аутентификация успешно завершена");

        // Даем системе время стабилизироваться после аутентификации
        println!("⏳ Даем системе время стабилизироваться после аутентификации...");
        sleep(Duration::from_secs(1)).await;

        // 9. XSTREAM ВЗАИМОДЕЙСТВИЕ
        ...
        // 10. ПЕРЕДАЧА ДАННЫХ ЧЕРЕЗ XSTREAM
        ...
        // 12. ПРОВЕРКА МОДИФИЦИРОВАННЫХ ДАННЫХ
        ...
        // 13. ОЖИДАНИЕ ЗАВЕРШЕНИЯ ЧТЕНИЯ ДАННЫХ НА НОДЕ1
        ...
        let received_bytes = match timeout(Duration::from_secs(1), data_receiver).await {
            Ok(Ok(data)) => {
                println!("✅ Оригинальные данные успешно получены через oneshot канал");
                data
            }
            Ok(Err(_)) => {
                panic!("❌ Ошибка получения оригинальных данных через oneshot канал");
            }
            Err(_) => {
        ...
        // 14. ПРОВЕРКА ЦЕЛОСТНОСТИ ОРИГИНАЛЬНЫХ ДАННЫХ
        ...
        // 15. GRACEFUL SHUTDOWN ОБЕИХ НОД
        ...
        println!("   - XStream успешно открыт");
        println!("   - Данные переданы через XStream");
