//! Интеграционный тест полного жизненного цикла ноды за 5 секунд

use std::time::Duration;
use tokio::time::timeout;
use xnetwork2::Node;

/// Тестирует полный жизненный цикл ноды: создание → запуск → прослушивание → событие → завершение
/// Весь тест должен укладываться в 5 секунд
#[tokio::test]
async fn test_node_lifecycle_in_5_seconds() {
    println!("🧪 Запуск интеграционного теста жизненного цикла ноды (5 секунд)...");

    // Таймаут на весь тест - 5 секунд
    let result = timeout(Duration::from_secs(5), async {
        // 1. СОЗДАНИЕ НОДЫ (0-1 секунда)
        println!("🆕 Создаем ноду...");
        let mut node = Node::new()
            .await
            .expect("❌ Не удалось создать ноду - критическая ошибка");

        println!("✅ Нода создана с PeerId: {}", node.peer_id());

        // 2. ПОДПИСКА НА СОБЫТИЯ ДО ЗАПУСКА
        println!("📡 Подписываемся на события ноды...");
        let mut events = node.subscribe();

        // 3. ЗАПУСК НОДЫ (1-2 секунды)
        println!("🚀 Запускаем ноду...");
        node.start()
            .await
            .expect("❌ Не удалось запустить ноду - критическая ошибка");

        println!(
            "✅ Нода запущена, состояние задачи: {}",
            node.get_task_status()
        );

        // 4. КОМАНДА LISTEN_ON (2-3 секунды)
        println!("🎯 Отправляем команду listen_on...");
        node.commander
            .listen_on("/ip4/127.0.0.1/udp/0/quic-v1".parse().unwrap())
            .await
            .expect("❌ Не удалось выполнить listen_on - критическая ошибка");

        println!("✅ Команда listen_on выполнена, ожидаем событие...");

        // 5. ОЖИДАНИЕ СОБЫТИЯ NewListenAddr (3-4 секунды)
        println!("⏳ Ожидаем событие NewListenAddr (таймаут 2 секунды)...");
        let listen_event = timeout(Duration::from_secs(2), async {
            loop {
                match events.recv().await {
                    Ok(event) => {
                        println!("📡 Получено событие: {:?}", event);
                        if let xnetwork2::node_events::NodeEvent::NewListenAddr { address, listener_id: _ } = event
                        {
                            return address;
                        }
                    }
                    Err(e) => panic!(
                        "❌ Ошибка получения события: {} - система событий не работает",
                        e
                    ),
                }
            }
        })
        .await
        .expect("❌ Таймаут ожидания события NewListenAddr - событие не пришло за 2 секунды");

        println!("✅ Получено событие NewListenAddr: {}", listen_event);

        // 6. ПРОВЕРКА ЦЕЛОСТНОСТИ (4-4.5 секунды)
        println!("🔍 Проверяем целостность данных...");

        // Проверяем, что адрес не пустой
        assert!(
            !listen_event.to_string().is_empty(),
            "❌ Получен пустой адрес прослушивания - система не работает"
        );

        // Проверяем формат адреса
        assert!(
            listen_event.to_string().contains("/ip4/127.0.0.1/udp/"),
            "❌ Неверный формат адреса: {} - система работает некорректно",
            listen_event
        );

        // Проверяем, что адрес содержит порт (система назначила реальный порт)
        assert!(
            listen_event.to_string().contains("/udp/"),
            "❌ Адрес не содержит порт: {} - система не назначила реальный порт",
            listen_event
        );

        println!("✅ Все проверки пройдены!");

        // 7. GRACEFUL SHUTDOWN (4.5-5 секунд)
        println!("🛑 Выполняем graceful shutdown...");
        node.commander
            .shutdown()
            .await
            .expect("❌ Не удалось выполнить graceful shutdown - критическая ошибка");

        println!("⏳ Ожидаем завершение фоновой задачи...");
        node.wait_for_shutdown()
            .await
            .expect("❌ Не удалось дождаться завершения ноды - критическая ошибка");

        println!("✅ Нода корректно завершила работу");

        // 8. ФИНАЛЬНАЯ ПРОВЕРКА
        assert_eq!(
            node.get_task_status(),
            "not_started",
            "❌ Нода не перешла в состояние 'not_started' после завершения"
        );
        assert!(
            !node.is_running(),
            "❌ Нода все еще работает после graceful shutdown"
        );

        println!("🎉 Интеграционный тест успешно завершен!");
    })
    .await;

    // Проверяем, что тест уложился в 5 секунд
    match result {
        Ok(_) => println!("✅ Тест выполнен за {} секунд - ВСЕГО 5 СЕКУНД!", 5),
        Err(_) => panic!("❌ ТЕСТ ПРЕВЫСИЛ ЛИМИТ В 5 СЕКУНД - ПРОБЛЕМА ПРОИЗВОДИТЕЛЬНОСТИ!"),
    }
}
