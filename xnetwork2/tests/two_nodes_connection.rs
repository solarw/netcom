//! Тест подключения двух нод: одна слушает, другая подключается
    ...
    timeout(timeout_duration, async {
        loop {
            match events.recv().await {
                ...
                Err(e) => {
                    return Err(format!("Ошибка получения события: {}", e).into());
                }
            }
        }
    })
...
/// Тестирует подключение двух нод: одна слушает, другая подключается
/// Весь тест должен укладываться в 5 секунд
#[tokio::test]
async fn test_two_nodes_connection_in_5_seconds() {
    println!("🧪 Запуск теста подключения двух нод (5 секунд)...");
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        // Проверяем, что оба события ConnectionEstablished получены
        ...
        // 9. GRACEFUL SHUTDOWN ОБЕИХ НОД (4.8-5 секунд)
        ...
        println!("🎉 Тест подключения двух нод успешно завершен!");
    }).await;
