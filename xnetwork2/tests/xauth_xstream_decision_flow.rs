//! Тест потока решений XAuth и XStream
...
#[tokio::test]
async fn test_xauth_xstream_decision_flow_in_5_seconds() {
    println!("🧪 Запуск теста потока решений XAuth и XStream (5 секунд)...");
    ...
    let result = timeout(Duration::from_secs(5), async {
        ...
        match node1_por_request {
            NodeEvent::VerifyPorRequest { peer_id,  .. } => {
        ...
        println!("🎉 Тест потока решений XAuth и XStream успешно завершен!");
    }).await;
