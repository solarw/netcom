//! Тест передачи ошибок при отклонении входящих XStream

use std::time::Duration;
use tokio::time::timeout;
use xnetwork2::node_events::NodeEvent;
use xnetwork2::{InboundDecisionPolicy, Node};

/// Тестирует передачу ошибок при отклонении входящих XStream
#[tokio::test]
async fn test_xstream_decision_error_propagation() {
    println!("🧪 Тестируем передачу ошибок при отклонении входящих XStream...");

    // Создаем две ноды с разными политиками принятия решений
    let mut node1 = Node::builder()
        .await
        .with_inbound_decision_policy(InboundDecisionPolicy::AutoApprove)
        .build()
        .await
        .expect("❌ Не удалось создать ноду1");

    let mut node2 = Node::builder()
        .await
        .with_inbound_decision_policy(InboundDecisionPolicy::ManualApprove)
        .build()
        .await
        .expect("❌ Не удалось создать ноду2");

    println!("✅ Созданы две ноды:");
    println!("   - Нода1: AutoApprove (автоматически одобряет все входящие потоки)");
    println!("   - Нода2: ManualApprove (требует ручного одобрения)");

    // Запускаем обе ноды
    node1.start().await.expect("❌ Не удалось запустить ноду1");
    node2.start().await.expect("❌ Не удалось запустить ноду2");

    // Нода1 слушает на localhost с QUIC
    node1
        .commander
        .listen_on("/ip4/127.0.0.1/udp/0/quic-v1".parse().unwrap())
        .await
        .expect("❌ Нода1 не смогла начать слушать");

    // Нода2 слушает на другом порту с QUIC
    node2
        .commander
        .listen_on("/ip4/127.0.0.1/udp/0/quic-v1".parse().unwrap())
        .await
        .expect("❌ Нода2 не смогла начать слушать");

    // Получаем адрес ноды1
    let node1_addr = node1
        .commander
        .get_network_state()
        .await
        .expect("❌ Не удалось получить состояние сети ноды1")
        .listening_addresses
        .first()
        .expect("❌ Нода1 не имеет адресов для прослушивания")
        .clone();

    println!("📡 Нода1 слушает на: {}", node1_addr);

    // Нода2 подключается к ноде1
    node2
        .commander
        .dial(node1.peer_id().clone(), node1_addr.clone())
        .await
        .expect("❌ Нода2 не смогла подключиться к ноде1");

    println!("✅ Нода2 подключилась к ноде1");

    // Ждем установления соединения
    tokio::time::sleep(Duration::from_millis(500)).await;

    // Нода1 пытается открыть XStream к ноде2 (ожидаем ошибку, так как нода2 требует ручного одобрения)
    println!("🔄 Нода1 пытается открыть XStream к ноде2 (ожидаем ошибку)...");

    let xstream_result = node1.commander.open_xstream(node2.peer_id().clone()).await;

    // Проверяем, что получили ошибку
    assert!(
        xstream_result.is_err(),
        "❌ Ожидалась ошибка при открытии XStream, но операция завершилась успешно"
    );

    let error = xstream_result.unwrap_err();
    println!("✅ Получена ожидаемая ошибка: {}", error);

    // Проверяем, что ошибка содержит информацию об отклонении
    let error_string = error.to_string();
    assert!(
        error_string.contains("ConnectionRefused")
            || error_string.contains("rejected")
            || error_string.contains("отклонен")
            || error_string.contains("unauthorized")
            || error_string.contains("auth"),
        "❌ Ошибка не содержит ожидаемой информации об отклонении: {}",
        error_string
    );

    println!("✅ Ошибка корректно передана от ноды2 к ноде1 через протокол XStream");

    // Останавливаем ноды
    node1
        .commander
        .shutdown()
        .await
        .expect("❌ Не удалось остановить ноду1");
    node2
        .commander
        .shutdown()
        .await
        .expect("❌ Не удалось остановить ноду2");

    println!("🎉 Тест передачи ошибок при отклонении входящих XStream пройден успешно!");
}

/// Тестирует успешное открытие XStream после одобрения
#[tokio::test]
async fn test_xstream_decision_approval_success() {
    println!("🧪 Тестируем успешное открытие XStream после одобрения...");

    // Создаем две ноды с разными политиками принятия решений
    let mut node1 = Node::builder()
        .await
        .with_inbound_decision_policy(InboundDecisionPolicy::AutoApprove)
        .build()
        .await
        .expect("❌ Не удалось создать ноду1");

    let mut node2 = Node::builder()
        .await
        .with_inbound_decision_policy(InboundDecisionPolicy::ManualApprove)
        .build()
        .await
        .expect("❌ Не удалось создать ноду2");

    println!("✅ Созданы две ноды:");
    println!("   - Нода1: AutoApprove (автоматически одобряет все входящие потоки)");
    println!("   - Нода2: ManualApprove (требует ручного одобрения)");

    // Запускаем обе ноды
    node1.start().await.expect("❌ Не удалось запустить ноду1");
    node2.start().await.expect("❌ Не удалось запустить ноду2");

    // Нода1 слушает на localhost с QUIC
    node1
        .commander
        .listen_on("/ip4/127.0.0.1/udp/0/quic-v1".parse().unwrap())
        .await
        .expect("❌ Нода1 не смогла начать слушать");

    // Нода2 слушает на другом порту с QUIC
    node2
        .commander
        .listen_on("/ip4/127.0.0.1/udp/0/quic-v1".parse().unwrap())
        .await
        .expect("❌ Нода2 не смогла начать слушать");

    // Получаем адрес ноды1
    let node1_addr = node1
        .commander
        .get_network_state()
        .await
        .expect("❌ Не удалось получить состояние сети ноды1")
        .listening_addresses
        .first()
        .expect("❌ Нода1 не имеет адресов для прослушивания")
        .clone();

    println!("📡 Нода1 слушает на: {}", node1_addr);

    // Создаем подписки на события до подключения
    let mut node1_events = node1.subscribe();
    let mut node2_events = node2.subscribe();

    // Нода2 подключается к ноде1
    node2
        .commander
        .dial(node1.peer_id().clone(), node1_addr.clone())
        .await
        .expect("❌ Нода2 не смогла подключиться к ноде1");

    println!("✅ Нода2 подключилась к ноде1");

    // Ждем события ConnectionEstablished на обеих нодах
    let (node1_conn_id, node2_conn_id) = tokio::join!(
        async {
            loop {
                match timeout(Duration::from_secs(5), node1_events.recv()).await {
                    Ok(Ok(NodeEvent::ConnectionEstablished { connection_id, peer_id })) => {
                        if peer_id == *node2.peer_id() {
                            return connection_id;
                        }
                    }
                    Ok(Ok(_)) => continue,
                    Ok(Err(_)) => panic!("❌ Канал событий ноды1 закрыт"),
                    Err(_) => panic!("❌ Таймаут ожидания соединения на ноде1"),
                }
            }
        },
        async {
            loop {
                match timeout(Duration::from_secs(5), node2_events.recv()).await {
                    Ok(Ok(NodeEvent::ConnectionEstablished { connection_id, peer_id })) => {
                        if peer_id == *node1.peer_id() {
                            return connection_id;
                        }
                    }
                    Ok(Ok(_)) => continue,
                    Ok(Err(_)) => panic!("❌ Канал событий ноды2 закрыт"),
                    Err(_) => panic!("❌ Таймаут ожидания соединения на ноде2"),
                }
            }
        }
    );

    println!("✅ Соединение установлено на обеих нодах:");
    println!("   Нода1 connection_id: {:?}", node1_conn_id);
    println!("   Нода2 connection_id: {:?}", node2_conn_id);

    // Запускаем аутентификацию для обеих нод
    println!("🔐 Запускаем аутентификацию в ручном режиме...");
    node1
        .commander
        .start_auth_for_connection(node1_conn_id)
        .await
        .expect("❌ Не удалось запустить аутентификацию для ноды1");
    node2
        .commander
        .start_auth_for_connection(node2_conn_id)
        .await
        .expect("❌ Не удалось запустить аутентификацию для ноды2");

    // Создаем задачи для обработки аутентификации на обеих нодах
    let node1_commander = node1.commander.clone();
    let node1_events = node1.subscribe();
    let node2_commander = node2.commander.clone();
    let node2_events = node2.subscribe();

    // Задача для обработки аутентификации на ноде1
    let auth_task1 = tokio::spawn(async move {
        println!("🔄 Нода1 ожидает аутентификацию...");

        let mut event_receiver = node1_events;

        loop {
            match timeout(Duration::from_secs(10), event_receiver.recv()).await {
                Ok(Ok(event)) => {
                    if let NodeEvent::VerifyPorRequest {
                        peer_id,
                        
                        ..
                    } = event
                    {
                        println!("✅ Нода1 получила запрос на аутентификацию от ноды2");

                        // Одобряем аутентификацию
                        let auth_result =
                            node1_commander.submit_por_verification(peer_id, true).await;
                        assert!(
                            auth_result.is_ok(),
                            "❌ Не удалось одобрить аутентификацию на ноде1"
                        );
                        println!("✅ Нода1 одобрила аутентификацию ноды2");
                        break;
                    }
                }
                Ok(Err(_)) => {
                    panic!("❌ Канал событий ноды1 закрыт");
                }
                Err(_) => {
                    panic!("❌ Таймаут ожидания аутентификации на ноде1");
                }
            }
        }
    });

    // Задача для обработки аутентификации и входящих запросов на ноде2
    let approval_task = tokio::spawn(async move {
        println!("🔄 Нода2 ожидает аутентификацию и входящий запрос на XStream...");

        let mut event_receiver = node2_events;

        // Сначала ждем запрос на аутентификацию
        loop {
            match timeout(Duration::from_secs(10), event_receiver.recv()).await {
                Ok(Ok(event)) => {
                    if let NodeEvent::VerifyPorRequest {
                        peer_id,
                        
                        ..
                    } = event
                    {
                        println!("✅ Нода2 получила запрос на аутентификацию от ноды1");

                        // Одобряем аутентификацию
                        let auth_result =
                            node2_commander.submit_por_verification(peer_id, true).await;
                        assert!(
                            auth_result.is_ok(),
                            "❌ Не удалось одобрить аутентификацию на ноде2"
                        );
                        println!("✅ Нода2 одобрила аутентификацию ноды1");
                        break;
                    }
                }
                Ok(Err(_)) => {
                    panic!("❌ Канал событий ноды2 закрыт");
                }
                Err(_) => {
                    panic!("❌ Таймаут ожидания аутентификации на ноде2");
                }
            }
        }

        // Теперь ждем запрос на XStream
        println!("🔄 Нода2 ожидает входящий запрос на XStream...");

        loop {
            match timeout(Duration::from_secs(10), event_receiver.recv()).await {
                Ok(Ok(event)) => {
                    if let NodeEvent::XStreamIncomingStreamRequest {
                        peer_id: _,
                        connection_id: _,
                        decision_sender,
                    } = event
                    {
                        println!("✅ Нода2 получила входящий запрос на XStream от ноды1");

                        // Одобряем запрос
                        let approve_result = decision_sender.approve();
                        assert!(
                            approve_result.is_ok(),
                            "❌ Не удалось одобрить входящий XStream"
                        );
                        println!("✅ Нода2 одобрила входящий XStream от ноды1");
                    }
                }
                Ok(Err(_)) => {
                    panic!("❌ Канал событий ноды2 закрыт");
                }
                Err(_) => {
                    panic!("❌ Таймаут ожидания входящего запроса на ноде2");
                }
            }
        }
    });

    // Ждем завершения аутентификации на обеих нодах
    auth_task1
        .await
        .expect("❌ Задача аутентификации ноды1 завершилась с ошибкой");
    println!("✅ Аутентификация на ноде1 завершена");

    // Даем время для завершения аутентификации
    tokio::time::sleep(Duration::from_millis(500)).await;

    // Нода1 пытается открыть XStream к ноде2 (ожидаем успех после одобрения)
    println!("🔄 Нода1 открывает XStream к ноде2 (ожидаем успех после одобрения)...");

    let xstream_result = timeout(
        Duration::from_secs(10),
        node1.commander.open_xstream(node2.peer_id().clone()),
    )
    .await;

    // Ждем завершения задачи одобрения
    approval_task.abort();
    let _ = approval_task.await;

    // Проверяем, что XStream успешно открыт
    assert!(xstream_result.is_ok(), "❌ Таймаут при открытии XStream");
    let xstream = xstream_result
        .unwrap()
        .expect("❌ Не удалось открыть XStream после одобрения");

    println!("✅ XStream успешно открыт после одобрения ноды2");

    // Проверяем, что XStream работает
    assert_eq!(
        xstream.peer_id,
        *node2.peer_id(),
        "❌ Неверный peer_id в XStream"
    );

    // Закрываем XStream
    drop(xstream);

    // Останавливаем ноды
    node1
        .commander
        .shutdown()
        .await
        .expect("❌ Не удалось остановить ноду1");
    node2
        .commander
        .shutdown()
        .await
        .expect("❌ Не удалось остановить ноду2");

    println!("🎉 Тест успешного открытия XStream после одобрения пройден успешно!");
}
