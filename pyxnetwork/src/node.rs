...
impl Node {
    ...
    fn info(&self) -> PyResult<String> {
        ...
        let running = self.runtime.block_on(async { *self.running.lock().await });

        Ok(format!("Node: Peer ID: {}, Running: {}", peer_id, running))
    }
    ...
    fn submit_por_verification<'py>(
        ...
        // Create a copy of data to move into the async block
        let py_data = data.map(|dict| dict.to_object(py));
        ...
        future_into_py(py, async move {
            ...
            // Create connection ID using new_unchecked
            let conn_id = libp2p::swarm::ConnectionId::new_unchecked(connection_id);
            ...
            let auth_result = Python::with_gil(
                |py| -> Result<xauth::definitions::AuthResult, PyErr> {
                    if accept {
                        ...
                        if let Some(obj) = py_data.as_ref() {
                        ...
                        let reason = if let Some(obj) = py_data.as_ref() {
                            let dict = obj.as_ref(py);
                            if let Ok(dict) = dict.downcast::<PyDict>() {
                                // Handle the Result<Option<&PyAny>, PyErr>
                                match dict.get_item("reason") {
                                    Ok(Some(msg_obj)) => {
            ...
            // Call submit_por_verification on the commander
            match cmd.submit_por_verification(conn_id, auth_result).await {
                Ok(()) => Ok(true),
                Err(e) => {
                    error!("Failed to submit PoR verification: {}", e);
                    Ok(false)
                }
            }
        })
