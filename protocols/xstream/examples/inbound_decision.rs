...
async fn main() -> Result<(), Box<dyn Error>> {
                    ...
                    event = server_swarm.select_next_some() => {
                        match event {
                            ...
                            SwarmEvent::Behaviour(event) => {
                                match event {
                                    XStreamEvent::InboundUpgradeRequest { peer_id, connection_id, response_sender } => {
                                        ...
                                        match decision {
                                            InboundDecision::Accept => {
                                                ...
                                                // Отправляем решение о разрешении
                                                if let Err(_) = response_sender.send(InboundUpgradeDecision::Approved) {
                                                    println!("⚠️  Не удалось отправить решение о разрешении");
                                                }
                                            }
                                            InboundDecision::Reject(reason) => {
                                                println!("❌ Сервер: Отклонен входящий апгрейд от {}: {}", peer_id, reason);
                                                // Отправляем решение об отклонении с причиной
                                                if let Err(_) = response_sender.send(InboundUpgradeDecision::Rejected(reason)) {
                                                    println!("⚠️  Не удалось отправить решение об отклонении");
                                                }
                                            }
                                        }
                                    }
    ...
    match server_stream_result {
        Ok(Ok(server_stream)) => {
            ...
            // Запускаем серверные операции - чтение запроса и отправка ответа
            ...
            // Запускаем клиентские операции - запись данных и чтение ответа
            let client_handle = tokio::spawn(async move {
                ...
                let response = client_stream.read_to_end().await.expect("Не удалось прочитать ответ");
                println!("📥 Клиент получил ответ: {}", String::from_utf8_lossy(&response));
        ...
        Ok(Err(_)) => {
            println!("❌ Сервер отклонил входящий поток (ожидаемое поведение для неавторизованных пиров)");
        }
        Err(_) => {
            println!("❌ Таймаут ожидания серверного потока - возможно, сервер отклонил запрос");
        }
    }
