...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match server_stream_result {
        Ok(Ok(server_stream)) => {
            ...
            // Запускаем клиентские операции - запись данных, сигнал EOF, чтение ответа и закрытие
            let client_handle = tokio::spawn(async move {
                ...
                println!("📤 Клиент отправляет запрос: {}", request_str);
                ...
                // Сигнализируем конец записи - паника при ошибке
                println!("📝 Клиент сигнализирует конец записи (EOF)...");
                client_stream.write_eof().await.expect("❌ ПАНИКА: Не удалось отправить EOF");
                println!("✅ EOF отправлен");

                // Читаем ответ от сервера - паника при ошибке
                let response = client_stream.read_to_end().await.expect("❌ ПАНИКА: Не удалось прочитать ответ от сервера");
                println!("📥 Клиент получил ответ: {}", String::from_utf8_lossy(&response));

                // Проверяем ответ - паника при несовпадении
                let expected_response = format!("Server response to authorized client: {}", request_str);
                if response != expected_response.as_bytes() {
                    panic!("❌ ПАНИКА: Получен неожиданный ответ! Ожидалось: '{}', получено: '{}'", 
                           expected_response, String::from_utf8_lossy(&response));
                }
                println!("✅ Ответ корректно проверен");
