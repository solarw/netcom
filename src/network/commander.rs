...
pub async fn listen_port(
    ...
    let host_str = host.as_ref().map_or_else(|| "0.0.0.0".to_string(), |h| h.clone());
    ...
    match response_rx.await? {
        ...
        Err(e) => {
            ...
            return Err(format!("Failed to listen on {}:{}: {}", 
                host.as_ref().map_or_else(|| "0.0.0.0".to_string(), |h| h.clone()), 
                port, e).into());
        }
    }
}
