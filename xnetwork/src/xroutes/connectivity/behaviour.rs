...
use super::relay_client::{RelayClientBehaviour, RelayClientBehaviourEvent, RelayClientCommand};
...
pub enum ConnectivityBehaviourEvent {
    RelayClient(RelayClientBehaviourEvent),
...
impl ConnectivityBehaviour {
    pub fn new(local_peer_id: PeerId) -> Self {
        Self {
            relay_client: RelayClientBehaviour::new(local_peer_id),
    ...
    pub fn handle_command(&mut self, command: ConnectivityCommand) {
        match command {
            ConnectivityCommand::RelayClient(relay_client_command) => {
                self.relay_client.handle_command(relay_client_command);
            }
    ...
    pub fn relay_client(&self) -> &RelayClientBehaviour {
        &self.relay_client
    }

    pub fn relay_client_mut(&mut self) -> &mut RelayClientBehaviour {
...
impl NetworkBehaviour for ConnectivityBehaviour {
    type ConnectionHandler = Either<
        <RelayClientBehaviour as NetworkBehaviour>::ConnectionHandler,
        <RelayServerBehaviour as NetworkBehaviour>::ConnectionHandler,
    >;
    ...
    fn handle_established_inbound_connection(
        ...
        // Try relay server first (for incoming relay requests)
        if self.relay_server.is_running() {
            match self.relay_server.handle_established_inbound_connection(connection_id, peer, local_addr, remote_addr) {
                Ok(handler) => return Ok(Either::Right(handler)),
                Err(_) => {
                    // Fall back to relay client
                    match self.relay_client.handle_established_inbound_connection(connection_id, peer, local_addr, remote_addr) {
                        Ok(handler) => return Ok(Either::Left(handler)),
                        Err(e) => return Err(e),
                    }
                }
            }
        } else {
            // Only relay client is available
            match self.relay_client.handle_established_inbound_connection(connection_id, peer, local_addr, remote_addr) {
                Ok(handler) => return Ok(Either::Left(handler)),
                Err(e) => return Err(e),
            }
    ...
    fn handle_established_outbound_connection(
        ...
        // Try relay client first (for outbound connections)
        match self.relay_client.handle_established_outbound_connection(connection_id, peer, addr, role_override, port_use) {
            Ok(handler) => return Ok(Either::Left(handler)),
            Err(_) => {
                // Fall back to relay server if running
                if self.relay_server.is_running() {
                    match self.relay_server.handle_established_outbound_connection(connection_id, peer, addr, role_override, port_use) {
                        Ok(handler) => return Ok(Either::Right(handler)),
                        Err(e) => return Err(e),
                    }
                } else {
                    return Err(ConnectionDenied::new("No relay handlers available"));
                }
            }
        }
    }
    ...
    fn on_swarm_event(&mut self, event: FromSwarm) {
        self.relay_client.on_swarm_event(event.clone());
        if self.relay_server.is_running() {
    ...
    fn on_connection_handler_event(
        ...
        match event {
            Either::Left(client_event) => {
                self.relay_client.on_connection_handler_event(peer_id, connection_id, client_event);
            }
            Either::Right(server_event) => {
                self.relay_server.on_connection_handler_event(peer_id, connection_id, server_event);
            }
        }
    }
    ...
    fn poll(
        ...
        // Poll the relay client behaviour
        loop {
            match self.relay_client.poll(cx) {
                Poll::Ready(ToSwarm::GenerateEvent(event)) => {
                    return Poll::Ready(ToSwarm::GenerateEvent(ConnectivityBehaviourEvent::RelayClient(event)));
                }
                Poll::Ready(other_event) => {
                    return Poll::Ready(other_event.map_out(|_| unreachable!()).map_in(Either::Left));
                }
                Poll::Pending => break,
            }
        }

        // Poll the relay server behaviour if running
        if self.relay_server.is_running() {
            loop {
                match self.relay_server.poll(cx) {
                    ...
                    Poll::Ready(other_event) => {
                        return Poll::Ready(other_event.map_out(|_| unreachable!()).map_in(Either::Right));
                    }
                    Poll::Pending => break,
                }
            }
...
impl From<RelayClientBehaviourEvent> for ConnectivityBehaviourEvent {
    fn from(event: RelayClientBehaviourEvent) -> Self {
        ConnectivityBehaviourEvent::RelayClient(event)
    }
}
