...
async fn test_real_dht_peer_discovery() {
    ...
    let connection_timeout = tokio::time::timeout(Duration::from_secs(10), async {
        for _ in 0..20 { // Проверяем события в течение 10 секунд
            tokio::select! {
    ...
    // 7. Даем время DHT сети установиться
    ...
    // 9. ГЛАВНЫЙ ТЕСТ: Client1 ищет Client2 через DHT
    println!("9️⃣ MAIN TEST: Client1 searching for Client2 via DHT...");
    
    let search_result = client1_commander.find_peer_addresses_advanced(client2_peer_id, 10).await;
    ...
    // 10. Обратный тест: Client2 ищет Client1
    println!("🔟 REVERSE TEST: Client2 searching for Client1 via DHT...");
    
    let reverse_search_result = client2_commander.find_peer_addresses_advanced(client1_peer_id, 10).await;
    ...
    // 11. Тест поиска bootstrap сервера (должен находиться легко)
    println!("1️⃣1️⃣ Testing search for bootstrap server...");
    
    let bootstrap_search = client1_commander.find_peer_addresses_advanced(bootstrap_peer_id, 5).await;
    
    match bootstrap_search {
        Ok(addresses) => {
            println!("   ✅ Found {} addresses for bootstrap server", addresses.len());
            assert!(!addresses.is_empty(), "Should definitely find bootstrap server");
        }
        Err(e) => {
            println!("   ❌ Failed to find bootstrap server: {}", e);
        }
    }
