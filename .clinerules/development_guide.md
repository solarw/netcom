...
- при добавлении новых файлов тестов, всегда проверяй что ты включаешь их в mod.rs  в противном случае они будут написаны, тесты будут запускаться нормально, но новые тесты исполняться не будут

- тесты, примеры всегда запускай через bash timeout, что бы избежать зависания, ограничивай вывод разумным количеством строк черех head, что бы избежать засорения вывода

 - работая со swarm  в тестах или примерах всегда надо убеждаться что swarm запущен, то event loop работает и сообщения обрабатываются

 - не выполнять блокирующих операций внутри обработки swarm.select_next_some(), никаких длительных await операций, никаких open_stream и работы с потоками внутри рабочего цикла!

 - во всех тестат с реальными сетевыми подключениями использовать quic! это единственный сетевой протокол который поддерживается
 - команды должны реализовываться с каналом ответа по которому можно знать что команда не только отправлена но и выполнена или была ошибка response: oneshot::Sender<Result<..., Box<dyn std::error::Error + Send + Sync>>>, и иметь alias в Commander 
